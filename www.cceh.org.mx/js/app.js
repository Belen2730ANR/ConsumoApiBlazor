...
async function descargarImagenBase64(guid, id) {
    try {
        const res = await fetch(`${CONFIG.URL_BASE}/api/ArchivoAgenda/ObtenerImagenAgendaExterna`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                Token: "", 
        ...
        if (resJson.status?.exito === 1 && resJson.data) {
            ...
            console.warn(`Imagen no encontrada para ID ${id}: ${resJson.status?.mensaje}`);
            document.getElementById(`img-cont-${id}`)?.classList.remove('skeleton');
        }
    } catch (e) {
        console.error("Error en la petición de imagen externa:", e);
    }
}
